<x-admin.layout>
    ...
    <div class="m-8 p-12 rounded border border-gray-200">
        <h1 class="font-semibold text-3xl text-gray-800">Edit User</h1>
        ...
        <form action="{{ route('admin.user.update', $user->id) }}" method="POST">
            ...
            <div class="mt-8 grid gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="username" id="username" class="bg-gray-100 border border-gray-200 rounded py-2 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Masukkan nama Anda" value="{{ $user->username }}" required/>
                    @error('username')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" class="bg-gray-100 border border-gray-200 rounded py-2 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="yourmail@provider.com" value="{{ $user->email }}" required/>
                    @error('email')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
