<x-admin.layout>
    ...
    <div class="m-8 p-12 rounded border border-gray-200">
        <h1 class="font-semibold text-3xl text-gray-800">Edit Pesan</h1>
        ...
        <form action="{{ route('admin.contact.update', $contact->id) }}" method="POST">
            ...
            <div class="mt-8 grid gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" class="bg-gray-100 border border-gray-200 rounded py-2 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Masukkan nama Anda" value="{{ $contact->name }}" required/>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" class="bg-gray-100 border border-gray-200 rounded py-2 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="yourmail@provider.com" value="{{ $contact->email }}" required/>
                    @error('email')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="message" id="message" rows="4" class="bg-gray-100 border border-gray-200 rounded py-2 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" placeholder="Tulis pesan Anda" required>{{ $contact->message }}</textarea>
                    @error('message')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
